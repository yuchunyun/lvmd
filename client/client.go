...
func (c *lvmConnection) GetLV(ctx context.Context, vgName string) ([]*lvmd.LogicalVolume, error) {
	...
	/*
		data := make([]string, len(rsp.GetVolumes()))
		if len(rsp.GetVolumes()) <= 0 {
			return []string{}, errors.New("Volume %v not exists")
		} else {
			fmt.Println("################")
			fmt.Println(rsp.GetVolumes())
			fmt.Printf("%T\n", rsp.GetVolumes())
			fmt.Println("################")
			for i := 0; i < len(rsp.GetVolumes()); i++ {
				jsonBytes, err := json.Marshal(rsp.GetVolumes()[i])
				if err != nil {
					return []string{}, err
				} else {
					data[i] = string(jsonBytes)
				}
			}
			return data, nil
		}*/
...
func (c *lvmConnection) GetVG(ctx context.Context) ([]*lvmd.VolumeGroup, error) {
	...
	/*
		if len(rsp.GetVolumeGroups()) <= 0 {
			return "", errors.New("Volume group %v not exists")
		}
		return rsp.GetVolumeGroups()[0].String(), nil
	*/
	return rsp.GetVolumeGroups(), nil

	/*
		data := make([]string, len(rsp.GetVolumeGroups()))
		if len(rsp.GetVolumeGroups()) <= 0 {
			return []string{}, errors.New("Volume group %v not exists")
		} else {
			fmt.Println("################")
			fmt.Println(rsp.GetVolumeGroups())
			fmt.Printf("%T\n", rsp.GetVolumeGroups())
			fmt.Println("################")
			for i := 0; i < len(rsp.GetVolumeGroups()); i++ {
				jsonBytes, err := json.Marshal(rsp.GetVolumeGroups()[i])
				if err != nil {
					return []string{}, err
				} else {
					data[i] = string(jsonBytes)
